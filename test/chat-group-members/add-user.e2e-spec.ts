    ...
    describe('Add User into Group', () => {
        it('should add a user into the group successfully with full payload', () => {
            const addUserDto: AddUserIntoGroupDto = {
                ...
                mutedUntil: new Date(Date.now() + 86400000).toISOString() as any, // 1 day later
            };
            ...
            return request(app)
                ...
                .expect(({ body }) => {
                    ...
                    expect(Number(body.groupId)).toBe(groupId);
                    expect(Number(body.userId)).toBe(addUserDto.userId);
                    expect(body.role).toBe(addUserDto.role);
                    expect(body.status).toBe(addUserDto.status);
                    expect(Number(body.lastReadMessageId)).toBe(addUserDto.lastReadMessageId);
